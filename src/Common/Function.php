...
/**
 * 快速缓存或读取Handlers
 * @param mixed $idOrHandlerOrFile 根据传入的数据类型不同，有不同的功能 
 * @return Array   如果传入的是HandlerID，则尝试返回该ID对应的handler集合
 *         TRUE    如果传入的是合法的Handler，则添加入Handler缓存
 *         FALSE   如果传入的是不合法的参数，则返回FALSE
 *         Array   如果不传参数，则返回以HandlerID作为Key的Handler集合
 * 	   Handler 如果传入的是Handler的全名（\NameSpace\HandlerClassName）
 * 		   则返回该Handler对象或者FALSE（Handler不存在）
 */
...
//	DILog('is_num '.$idOrHandler);
    ...
    elseif (is_string($idOrHandler))
    {
	if (isset($_handlers[$idOrHandler]))
	{
	    return $_handlers[$idOrHandler];
	}
	else
	{
	    return FALSE;
	...
	if (is_numeric($handler->ID()))
	{
	    ...
	    $_handlers[get_class($handler)] = &$handler; //每个Handler在各个进程只会被实例化1次
