...
class Build
{
	...
	if (is_writeable(DI_APP_PATH))
	{
	    ...
	    Build::makeFunctions();//生成基础函数
	    Build::makeReloadHelper($serverName);//生成基础热重载类。
	}
	else
	{
	    header('Content-Type:text/html; charset=utf-8');
	    exit('应用目录[' . DI_APP_PATH . ']不可写，目录无法自动生成！<BR>请手动生成项目目录~');
	}
    }
    ...
    {
	//生成Server方法文件
	if (!is_file(DI_APP_SERVER_COMMON_PATH . '/Function.php'))
	    file_put_contents(DI_APP_SERVER_COMMON_PATH. '/Function.php', "<?php\n/*\n *这里的方法不会被热重载，仅在服务被加载的时候生效。请将与Server有关的方法放在这里\n */");
	//生成Worker方法文件
	if (!is_file(DI_APP_SERVER_WORKER_COMMON_PATH . '/Function.php'))
	    file_put_contents(DI_APP_SERVER_WORKER_COMMON_PATH. '/Function.php', "<?php\n/*\n *这里的方法会在ServerReload的时候被热重载\n请将与业务有关的方法放在这里\n */");
    }

    static private function makeConfigs()
    {
	// 写入Server配置文件
	if (!is_file(DI_APP_SERVER_CONF_PATH . '/Config' . CONF_EXT))
	    file_put_contents(DI_APP_SERVER_CONF_PATH . '/Config' . CONF_EXT, '.php' == CONF_EXT ? "<?php\n//这里的配置不会被热重启，仅在服务被加载的时候生效\n//请将与Server有关的配置放在这里\nreturn [\n\t//'配置项'=>'配置值'\n".
		    "\t'DI_LISTENERS' => [\n\t\t['Host' => '0.0.0.0', 'Port' => '5200', 'Type' => SWOOLE_SOCK_UDP]\n\t]//其他配置项请参考惯例配置文件\n"
		    ."];" : '');
	// 写入Worker配置文件
	if (!is_file(DI_APP_SERVER_WORKER_CONFIG_PATH . '/Config' . CONF_EXT))
	    file_put_contents(DI_APP_SERVER_WORKER_CONFIG_PATH . '/Config' . CONF_EXT, '.php' == CONF_EXT ? "<?php\n//这里的配置会在ServerReload的时候被热重载\n//请将与业务有关的配置放在这里\nreturn [\n\t//'配置项'=>'配置值'\n];" : '');
    }
