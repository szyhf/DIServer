 ...
 * @return string
 */
function parse_name_nativecase($name, $type = 0)
{
...
function S_bak($name = '', $value = '', $options = null)
{
    static $cache = '';
    if (is_array($options) && empty($cache))
    {
	// 缓存操作的同时初始化
	$type = isset($options['type']) ? $options['type'] : 'KVStore';
	$cache = new \DIServer\KVStore($options);
    }
    ...
    if (empty($name) AND empty($value) AND is_null($options))
    {
 ...
 * @return mixed
 */
function S($name=null, $value = '', $options = null)
{
    static $cache = '';
    if (is_array($options) && empty($cache))
    {
