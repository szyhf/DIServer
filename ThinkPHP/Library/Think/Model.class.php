...
class Model
{
	...
	if (empty($this->fields))
	{
		...
		$fields = F('_fields/' . ($db . '.' . $this->tablePrefix . $this->name));
		if ($fields)
		{
		    $this->fields = $fields;
		    if (!empty($fields['_pk']))
	...
	if ((strpos($field, ',')||is_array($field)) && false !== $sepa)
	{ // 多字段
	...
	if (empty($this->trueTableName))
	{
	    ...
	    {
		$tableName .= trim($this->name); //parse_name会指向Common/functions.php，里边会强制把字符串转换为小写
		//$tableName = trim(preg_replace("/[A-Z]/", "_\\0", $this->$name), "_");
	    }
	    //$this->trueTableName    =   strtolower($tableName);//默认访问数据表会不区分大小写
