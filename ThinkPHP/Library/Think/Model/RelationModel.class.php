...
// +----------------------------------------------------------------------
	...
	{	    
		...
		foreach ($this->_link as $key => $val)
		{
		    ...
		    if (empty($name) || true === $name ||$key==$name|| $mappingName == $name || (is_array($name) && in_array($mappingName, $name)))
		    {
	...
	protected function opRelation ($opType, $data = '', $name = '')
	{
		...
		foreach ($this->_link as $key => $val)
		{
			...
			if (isset($val['as_fields']) && in_array($mappingType, array (self::HAS_ONE, self::BELONGS_TO)))
			{
			    //1 将as_field映射还原成relationData(即按mapping_field顺序存储的字典）
			    $relationData = null;
			    $fields = explode(',', $val['as_fields']);
			    foreach ($fields as $field)
			    {
				...
				{
				    list($relationName, $nick) = explode(':', $field);
				    $relationData[$relationName] = $data[$nick];
				    unset($data[$nick]);
				}
				else
				{
