...
abstract class Driver
{
    ...
    // 数据库表达式
    protected $exp = array('eq' => '=', 'neq' => '<>', 'gt' => '>', 'egt' => '>=', 'lt' => '<', 'elt' => '<=', 'notlike' => 'NOT LIKE', 'like' => 'LIKE',
	'in' => 'IN',
	'notin' => 'NOT IN', 'not in' => 'NOT IN', 'between' => 'BETWEEN', 'not between' => 'NOT BETWEEN', 'notbetween' => 'NOT BETWEEN');
    // 查询表达式
     ...
     * @access public
     */
    public function forceReConnect($config = '', $linkNum = 0, $autoConnection = false)
    {
	if (empty($config))
	    $config = $this->config;
	try
	...
	for ($i = 0; $i <= C('DB_RECONNECT_TIMES'); $i++)
	{
	    ...
	    {
		$eroInfo = $ex->errorInfo;
		if ($eroInfo[1] == 2006 || $eroInfo[1] == 2013)
		{
		    if (IS_CLI && APP_DEBUG)
		    {
			DILog("PDODriver has gone and try reconnect\n");
		    }
		    sleep(C('DB_RECONNECT_WAIT'));
		    $this->_linkID = $this->forceReConnect();
		    continue; //重试
     ...
     * @param boolean $force 强制重连
     * @return void
     */
    protected function initConnect($master = true, $force = false)
    {
